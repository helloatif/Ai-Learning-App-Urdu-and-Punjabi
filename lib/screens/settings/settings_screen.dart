...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: isDark ? AppTheme.darkBackground : const Color(0xFFF5F7FA),
  ...
  Widget _buildAvatar(UserProvider userProvider) {
    ...
    return _buildAvatarImage(
      selectedPath,
      fallbackIcon: selectedPath == _girlAvatarPath ? Icons.face_3 : Icons.face_6,
      width: double.infinity,
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('AI Tools', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            _buildToolButton(
              context,
              ...
              icon: Icons.rule,
              onPressed: () {
                Navigator.of(context).push(MaterialPageRoute(builder: (_) => const GrammarCheckerScreen()));
              },
            ),
            const SizedBox(height: 8),
            _buildToolButton(
              ...
              onPressed: () {
                Navigator.of(context).push(MaterialPageRoute(builder: (_) => const TranslationPracticeScreen()));
              },
            ),
            const SizedBox(height: 8),
            _buildToolButton(
              ...
              icon: Icons.mic,
              onPressed: () {
                Navigator.of(context).push(MaterialPageRoute(builder: (_) => const VoiceAssistantScreen()));
              },
            ),
          ],
        ),
  ...
  Widget _buildToolButton(BuildContext context, {required String label, required IconData icon, required VoidCallback onPressed}) {
    return SizedBox(
      width: double.infinity,
  ...
  Widget _buildAvatarImage(String path, {required IconData fallbackIcon, required double width, required double height}) {
    return Image.asset(
      path,
      width: width,
      height: height,
  ...
  Widget _buildAvatarChoice({required UserProvider userProvider, required String value, required String label, required String assetPath}) {
    final bool isSelected = userProvider.selectedAvatarPath == value;

    return GestureDetector(
      onTap: () => userProvider.setSelectedAvatarPath(value),
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isSelected ? AppTheme.primaryGreen : Colors.grey.withValues(alpha: 0.35),
            ...
            Container(
              ...
              child: ClipOval(
                child: _buildAvatarImage(
                  assetPath,
                  fallbackIcon: value == _girlAvatarPath ? Icons.face_3 : Icons.face_6,
                  width: 60,
                  height: 60,
                ),
              ),
  ...
  Widget _buildEmailVerificationStatus(BuildContext context) {
    ...
    return Card(
      color: isVerified ? AppTheme.lightGreen.withValues(alpha: 0.2) : Colors.orange.withValues(alpha: 0.2),
      child: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Row(
          children: [
            Icon(isVerified ? Icons.verified : Icons.warning, color: isVerified ? AppTheme.primaryGreen : Colors.orange),
            const SizedBox(width: 16),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(isVerified ? 'Email Verified ✓' : 'Email Not Verified', style: const TextStyle(fontWeight: FontWeight.bold)),
                  if (!isVerified) const Text('Please check your email', style: TextStyle(fontSize: 12)),
                ],
              ),
            ),
            if (!isVerified)
      ...
      builder: (context, themeProvider, _) {
        return Card(
          child: SwitchListTile(
            secondary: Icon(themeProvider.isDarkMode ? Icons.dark_mode : Icons.light_mode, color: AppTheme.primaryGreen),
            title: const Text('Dark Mode', style: TextStyle(fontWeight: FontWeight.bold)),
            subtitle: Text(themeProvider.isDarkMode ? 'On' : 'Off', style: const TextStyle(fontSize: 12)),
  ...
  Widget _buildLanguageSwitcher(BuildContext context, UserProvider userProvider) {
    final currentLanguage = userProvider.currentUser?.selectedLanguage ?? 'urdu';
    ...
    return Card(
      ...
      child: ListTile(
        ...
        onTap: () => _showLanguageDialog(context, userProvider, currentLanguage),
      ),
    );
  }

  void _showLanguageDialog(BuildContext context, UserProvider userProvider, String currentLanguage) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
          ...
          children: [
            ListTile(
              ...
              onTap: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Language changed to Urdu')));
              },
            ),
            const SizedBox(height: 8),
            ListTile(
              ...
              onTap: () {
                ...
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Language changed to Punjabi')));
              },
            ),
          ],
        ),
